...
public final class CombatWorldRenderer implements Disposable {
    ...
    private void drawSparseHazards(SpriteBatch batch, CombatWorldStyle.Profile profile, TextureRegion hazard) {
        ...
        int tileBudget = Math.max(2, Math.min(3, Math.round(260f * profile.hazardCoverage())));
        ...
        for (int i = 0; i < tileBudget; i++) {
            ...
            if (horizontalEdge) {
                ...
                y = ((i & 2) == 0 ? -1f : 1f) * (15.4f + CombatWorldStyle.unit(h >>> 11) * 1.0f);
            } else {
                x = ((i & 2) == 0 ? -1f : 1f) * (28.0f + CombatWorldStyle.unit(h) * 1.2f);
    ...
    private static void setHazardTint(SpriteBatch batch, EnvironmentBiomeRules.Biome biome) {
        switch (biome) {
            case CINDER_FOUNDRY -> batch.setColor(1f, .42f, .10f, .28f);
            case NULL_SECTOR -> batch.setColor(.54f, .32f, 1f, .24f);
            case CRYO_VAULT -> batch.setColor(.36f, .82f, 1f, .24f);
            case CRYOGENIC_DEPTHS -> batch.setColor(.18f, .64f, .76f, .22f);
            default -> batch.setColor(.92f, .66f, .16f, .24f);
        }
    }
